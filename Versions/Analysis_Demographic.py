...
# As the Correlation between the variables are very less we can move forward with the Prediction 

# Predicting the Renewed and Non Renewed Data for 2017 
X = Final_train_Data

from sklearn.linear_model import LogisticRegression
log_reg = LogisticRegression(solver='lbfgs',multi_class='multinomial', C=1000, tol=0.0008)
...
test_results = test 
test_results.groupby(['Subscription_Status'])['Expiring_Dollars'].count().plot(kind="pie" ,autopct='%1.1f%%', title= "2017 Renew Data")
...
Data_final_2016['Subs__End_Date'] = Data_final_2016['Subs__End_Date'].apply(pd.to_datetime)
Data_final_2017['Subs__End_Date'] = Data_final_2017['Subs__End_Date'].apply(pd.to_datetime)

Data_final_2016['month'] = pd.DatetimeIndex(Data_final_2016['Subs__End_Date']).month
Data_final_2017['month'] = pd.DatetimeIndex(Data_final_2017['Subs__End_Date']).month

         
Data_final_2017.groupby(['month'])['Expiring_Dollars'].mean().plot(kind="line" , color="Red" )
Data_final_2016.groupby(['month'])['Expiring_Dollars'].mean().plot(kind="line" , color="Green")
